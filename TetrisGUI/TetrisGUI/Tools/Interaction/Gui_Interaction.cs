...
namespace TetrisGui.Tools.Interaction
{
    class Gui_Interaction : GuiInteraction
    {
            ...
            this.ActionQueue.Add(new Action(() =>
                {
                    ...
                    MainWindow.label_EARTHTIME.Content = "EARTH (Berlin): " + System.Environment.NewLine + DateTime.Now.ToLocalTime().ToLongTimeString();
        ...
        public void ShowHighScore_XML(string Name, string Score)
        {
            if (this.Game.GameIsOver == true &&
                         NoMoreLoops == false)
            {
                //  foreach (HighScore hs in this.Game.Config.)
                string _Highscore = "\t TOP 15 | HIGHSCORES:                                (XML)" + System.Environment.NewLine;



               
                }
                MainWindow.label_HighScore.Content = "AAAAAAAAAAAAAAAAAAAA";
